...
class MultimodalSearch(AnalysisMethod):
    ...
    def _save_embeddings(
        ...
        if isinstance(embeddings, torch.Tensor):
            ...
            if out_path.exists() and not overwrite:
                raise FileExistsError(f"File already exists: {out_path}. Use overwrite=True to overwrite.")
            ...
            raise ValueError("Embeddings must be either a torch.Tensor or a numpy.ndarray.")
    ...
    def _save_faiss_index(self, 
        ...
        if self.model.device == "cuda":
            faiss_index = faiss.index_gpu_to_cpu(self.faiss_index) # convert to cpu index for saving
        else:
            faiss_index = self.faiss_index
        
        faiss.write_index(faiss_index, str(out_path))

    def _save_image_paths(self, 
                          save_path: Optional[Union[str, Path]] = None, 
    ...
    def index_images(
            ...
            if not images:
                ...
                images = [
                    v["filename"] for v in self.subdict.values()
                    if isinstance(v, dict) and "filename" in v
                ]

            images_list, image_paths = self._prepare_images(images)
            self.image_paths = image_paths
            self._default_save_dir = Path(save_path).resolve() if save_path else Path(image_paths[0]).parent
            ...
            if save_embeddings_and_indexes:
                self._save_embeddings(self.image_embeddings, self._default_save_dir, overwrite=overwrite)
    ...
    def _load_embeddings(
        ...
        if path.suffix == '.npy':
            ...
            if np_path.exists() and self.model.device == "cpu":
                ...
                raise FileNotFoundError(f"No embeddings file found in directory: {path}")
        else:
            raise ValueError("Unsupported embedding file format. Use .npy or .pt files.")
    ...
    def _load_faiss_index(
        ...
        if self.model.device == "cpu":
            self.faiss_index = faiss.read_index(str(path))
                ...
                self.faiss_index = faiss.index_cpu_to_gpu(res, flat_config.device, cpu_index)
    ...
    def multimodal_search(
        ...
        if  self.faiss_index is None:
            raise RuntimeError("No image faiss_indexes found. Call index_images first or set load_indexes to True.")
        
        if return_paths and not self.image_paths:
            raise RuntimeError("No image paths found. Cannot return paths. Ensure image paths are loaded.")
        ...
        return self._search_embeddings(q_emb, top_k=top_k, score_threshold=score_threshold, return_paths=return_paths)
